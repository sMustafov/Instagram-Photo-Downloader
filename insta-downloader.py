...
def download_image():
    ...
    try:
        if x:
            ...
            if final == "video": 
                print("You cannot download a video file")
...
# Download profile picture of instagram accounts
def profile_photo_download():
...
if connection() == True:
    try:
        while True:
            a = "Press 'A' to download an instagram profile photo.\nPress 'B' to download an instagram image.\nPress 'Q' to exit."
            print(a)
            select = str(input("\nInsta Photo Downloader > ")).upper()
            try:
                if select == 'A':
